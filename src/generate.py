...
def generate_pages_recursively(dir_path_content, template_path, dest_dir_path, basepath, ):
    ...
    public_path = pathlib.Path(dest_dir_path)
    ...
    for f in os.listdir(content_path):
        ...
        if os.path.isfile(content_file_path):
            ...
            generate_page(content_file_path, template_path, public_file_path, basepath)
        else:
            # rel_content_path = f"{dir_path_content}/{f}"
            # rel_public_path = f"{dest_dir_path}/{f}"
            rel_content_path = os.path.join(dir_path_content, f)
            rel_public_path = os.path.join(dest_dir_path, f)
            generate_pages_recursively(rel_content_path, template_path, rel_public_path, basepath)
